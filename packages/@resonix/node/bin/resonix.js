...
const matrix = {
	...
	linux: { x64: '@resonix/linux-x86_64', arm64: '@resonix/linux-aarch64', arm: '@resonix/linux-armv7' }
};
...
function pickBinary() {
	...
	if (!binDir) {
		...
		const archHint = process.arch === 'arm64' ? 'aarch64' : (process.arch === 'arm' ? 'armv7' : 'x86_64');
		const prioritized = [...candidates].sort((a, b) => {
			const score = n => (n.includes(platformHint) ? 2 : 0) + (n.includes(archHint) ? 1 : 0);
			return score(b) - score(a);
		});
		for (const name of prioritized) {
			binDir = resolveBinaryDir(name);
			if (binDir) { chosen = name; break; }
		}
	}
	if (!binDir) return null;
	const entries = (() => { try { return fs.readdirSync(binDir); } catch { return []; } })();
...
if (!resolved) {
	console.error('[resonix] Unable to locate a platform-specific binary. Did installation skip optional deps?');
	process.exitCode = 1;
	return;
}
